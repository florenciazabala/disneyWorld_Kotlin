			...
			public static void inserta(ColeccionVO c) {
				...
				try {
					...
					if (existe(c.getCodigo()) == true) {
					       System.out.println("La colección ya se encuentra registrada");
					}else {
						System.out.println("Se registró correctamente a la colección "+c.getNombreColeccion());
					}
					con.commit();//2- Transacción
				}catch(SQLException e) {
					System.out.println("No es posible insertar a la colección "+c.getNombreColeccion() );
			...
			public static ColeccionVO getColeccionVO2 (String codigo) throws NegocioException, CafeStoreException {
				...
				boolean existe = false;
					...
					while (rs.next()) {
						...
						c.setDescripcion(descripcion);
						if(existe(c.getCodigo()) == true) {
							System.out.println("Los datos de la coleccion que corresponde a su búsqueda son: ");
							System.out.println(c.toString());
						}
						}
					if(existe(c.getCodigo()) == false) {
			...
			public static ColeccionVO getColeccionVOg (String busqueda) throws NegocioException, CafeStoreException {
				...
				boolean existe = false;
					...
					while (rs.next()) {
						...
						c.setDescripcion(descripcion);
						if(existe(c.getCodigo()) == true) {
							System.out.println("Los datos de la coleccion que corresponden a su bésqueda son : ");
							System.out.println(c.toString());
						}
						}
					if(existe(c.getCodigo()) == false) {
